...
export default {
  ...
  methods: {
    async getDataFromApi() {
      ...
      await http
        .get(`/qnaboard/pagelink/${limit}/${this.offset}`)
        .then(({ data }) => {
          console.log(data);
          this.items = data;
          this.loading = false;
    ...
    handleClick(value) {
      this.$router.push({
        name: 'board',
        params: { no: value.boardNum, boardType: 'qna', offset: this.offset, page: this.page },
      });
    },
    goSearch() {
      console.log('hi');
    ...
    moveReg() {
      this.$router.push({
        name: 'boardReg',
        params: { boardType: 'qna', offset: this.offset, page: this.page },
      });
    },
  },
  created() {
    http
      .get('/qnaboard/pagelink/totalCnt')
      .then(({ data }) => {
